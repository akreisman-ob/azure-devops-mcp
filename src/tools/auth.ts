...
async function getCurrentUserDetails(tokenProvider: () => Promise<AuthToken>, connectionProvider: () => Promise<WebApi>, userAgentProvider: () => string) {
 ...
 */
async function searchIdentities(identity: string, tokenProvider: () => Promise<AuthToken>, connectionProvider: () => Promise<WebApi>, userAgentProvider: () => string): Promise<IdentitiesResponse> {
  const authToken = await tokenProvider();
  const connection = await connectionProvider();
 ...
 */
async function getUserIdFromEmail(userEmail: string, tokenProvider: () => Promise<AuthToken>, connectionProvider: () => Promise<WebApi>, userAgentProvider: () => string): Promise<string> {
